...
├── optional/                  # Optional modular installation system
│   ├── install_mods/          # Installation module scripts
│   ├── overlay-includes/      # Optional module overlay files
│   └── overlay-config/        # Optional module configurations
...
Our approach to package selection emphasizes security and reliability through explicit inclusion of critical components:

- **Base System Integration**:
    * Explicitly include crucial security packages even when part of Debian standard system
    * Ensures consistent security tooling across different Debian-based environments
    * Prevents variations in package availability between distributions

- **Core Security Packages**:
    * Essential security tools bundled in `core-basic-security.list.chroot`
    * System integrity monitoring (AIDE, checksecurity)
    * Access control and auditing (acl, auditd)
    * Filesystem monitoring (inotify-tools, fatrace)
    * Malware protection (ClamAV)

- **Critical Service Packages**:
    * Explicit inclusion of core scheduling services (cron, anacron)
    * Ensures availability of automated security tasks
    * Maintains consistent system maintenance capabilities

This strategy provides a reliable foundation where security-critical functionalities are guaranteed to be present, regardless of the base system's package selection or priority levels.
...
- Optional modules
2. For optional module contributions:
- Review Documentation/04-Modular-Install-Modifier-System.md
- Place installation scripts in optional/install_mods/
- Add any required overlay files in optional/overlay-includes/
- Include necessary configurations in optional/overlay-config/
- Test the module using build.sh with appropriate --install-modulename flag (e.g., --install-warp)
...
# For optional module additions/changes
git checkout -b optional/module-name
```
